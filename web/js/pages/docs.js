$(document).ready(function (e) {
    ...
    $('#if').attr('src','https://docs.google.com/document/d/'+document_id+'/edit');
            ...
            connection.socket.on(connection.socketCustomEvent, function(event) {
                if(event.accion == 'vco') {
                    ...
                    if(event.destinatario == participante){
                        ...
                        console.log("respuesta pantalla: "+currentscreen+" "+event.destinatario);
                        if(currentscreen == 'invitar' || currentscreen == 'consultores'){
                            currentscreen = 'index';
                        }
                        if(currentscreen != 'docs' || (getParameterByName('document_id') == null)){
...
//EMITIR LLAMADO A PARTICIPANTES PARA QUE CAMBIEN PANTALLA
if(tipo == 'consultor'){
    console.log('cambien pantalla '+document_id);
    connection.socket.emit(connection.socketCustomEvent, {
        accion: 'docs',
        document_id: document_id,
    });
